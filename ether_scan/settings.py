# Define list of fixed parameters or constants

ETHERSCAN_API_KEY = r'U2AVBFFQBARSXV42JM687S8XZGVREEM78G'
WALLET_TRANSACTION_URL = r'https://api.etherscan.io/api'
TRANSACTION_DETAILS_URL = r''
BLOCK_DETAILS_URL = r'https://etherscan.io/block'
APPROVED_TOKENS = ['ETH', 'BTC', 'PAX', 'XRP']
APPROVED_FIATS = ['USD', 'EUR', 'GBP', 'HKD', 'YEN', 'NGN']
CRYPTO_COMPARE_API = ''
CRYPTO_COMPARE_SINGLE_SYMBOL_PRICE = r'https://min-api.cryptocompare.com/data/price'
CRYPTO_COMPARE_OHLCV_BY_TS = r'https://min-api.cryptocompare.com/data/pricehistorical'
BLOCKCYPHER_BALANCE_TRANSACTION_URL = r'https://api.blockcypher.com/v1/eth/main/addrs'
BLOCKCYPHER_TRANSACTION_DETAILS_URL = r'https://api.blockcypher.com/v1/eth/main/txs'
BLOCKCYPHER_BLOCK_DETAILS_URL = r'https://api.blockcypher.com/v1/eth/main/blocks'
